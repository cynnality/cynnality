/* gradientGen.js (patched)
 * Exposes: window.GradientGen = { generateTheme, precompute, defaults }
 * Requires: p5.js already loaded (uses lerp(), constrain(), random())
 */
(function () {
  // ---------- Small helpers ----------
  ...
  // Pure HSV -> RGB (no p5 colorMode flipping)
  // h: 0..360, s: 0..100, v: 0..100  -> [r,g,b] 0..255
  function hsvToRGB_math(h, s, v) {
    s /= 100; v /= 100;
    const c = v * s;
  ...
  const DEFAULTS = {
    ...
    minVStep: 1.2,  // vertical monotone brightness step (per row down a column)
    ...
    bandStepRange: [10, 18],         // hue band offset per band
    vBandStepRange: [0, 0],          // NEW: tiny per-band V bump (e.g. [-3, 3]) (0 = off)
    ...
    // NEW: optional global light/dark separation helpers (all no-ops by default)
    vColSpanMin: 0,    // ensure a minimum top->bottom V span per column (e.g. 10..16)
    vContrast: 1.0,    // amplify V contrast around mid (>1 = more contrast)
    vGamma: 1.0        // gamma shaping on V (1 = off). <1 brightens, >1 darkens
  };
  ...
  function generateTheme() {
    ...
    // Use p5's random for S/V ranges (consistent with existing usage)
  ...
  function precompute(rows, cols, topA, topB, botA, botB, options = {}) {
    ...
    const vBandStep   = random(opt.vBandStepRange[0], opt.vBandStepRange[1]); // can be 0..0 (no-op)
    ...
    for (let r = 0; r < rows; r++) {
      ...
      for (let c = 0; c < cols; c++) {
        ...
        // optional tiny per-band V bump (helps separate adjacent hue bands)
        if (opt.vBandStepRange[0] !== 0 || opt.vBandStepRange[1] !== 0) {
          v += bandIdx * vBandStep;
        }
    ...
    // 2a) Ensure minimum top->bottom V span per column (optional)
    if (opt.vColSpanMin > 0 && rows > 1) {
      const midV = 0.5 * (opt.minV + opt.maxV);
      for (let c = 0; c < cols; c++) {
        const top = V[0][c], bot = V[rows-1][c];
        let span = Math.abs(bot - top);
        if (span < opt.vColSpanMin) {
          const need = opt.vColSpanMin / (span || 1); // scale around midV
          for (let r = 0; r < rows; r++) {
            V[r][c] = constrain(midV + (V[r][c] - midV) * need, opt.minV, opt.maxV);
          }
        }
      }
    }
    ...
    // 4) Convert to RGB + optional global contrast/gamma + enforce uniqueness
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        // global contrast around mid (optional, no-op at 1.0)
        if (opt.vContrast && opt.vContrast !== 1.0) {
          const mid = 0.5 * (opt.minV + opt.maxV);
          v = constrain(mid + (v - mid) * opt.vContrast, opt.minV, opt.maxV);
        }
        // optional gamma shaping
        if (opt.vGamma && opt.vGamma !== 1.0) {
          const vv = v / 100;
          v = Math.pow(vv, opt.vGamma) * 100;
        }

        let rgb = hsvToRGB_math(h, s, v);
